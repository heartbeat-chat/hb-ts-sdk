...
const users: Heartbeat.UserListResponse = await client.users.list();
```
...
<!-- prettier-ignore -->
```ts
const users = await client.users.list().catch(async (err) => {
  if (err instanceof Heartbeat.APIError) {
...
const response = await client.users.list().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: users, response: raw } = await client.users.list().withResponse();
...
1. Via the `HEARTBEAT_LOG` environment variable
...
const client = new Heartbeat({
  logger: logger.child({ name: 'Heartbeat' }),
